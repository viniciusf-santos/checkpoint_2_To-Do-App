...
function rederizaTarefas(listaTarefas) {
  ...
  for (let tarefa of listaTarefas) {
    ...
    if (!tarefa.completed) {
      ...
      let li = document.createElement("li");
      li.classList.add("tarefa");
      li.id = `tarefa-terminada-${tarefa.id}`;
      li.innerHTML = `
                    <div class="descricao">
                    <p class="nome">${tarefa.description}</p>
                    <div class="opcoes-tarefas-completas">
                        <button onclick ="retornarTarefa('${tarefa.id}')"><i class="fas fa-undo-alt change" title="Voltar para tarefa pendente"></i></button>
                        <button onclick ="deletarTarefaById('${tarefa.id}')"><i class="far fa-trash-alt"></i></button>
                    </div>
                </div>
            `;
...
function moveTarefaParaTerminada(tarefa) {
  ...
  tarefaTerminada.setAttribute("id", `tarefa-terminada-${tarefa.id}`);
...
function deletarTarefaById(id) {
  fetch(`${apiBaseUrl()}/tasks/${id}`, {
    ...
    .then((response) => {
      document.getElementById(`tarefa-terminada-${id}`).remove();
      location.reload();
    }).catch;
}
